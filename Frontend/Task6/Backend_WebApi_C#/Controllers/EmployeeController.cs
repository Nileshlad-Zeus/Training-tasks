...
namespace Backend2.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<IActionResult> Updatevalue([FromBody] UpdateData request)
        {
            ...
            {
                await _connection.OpenAsync();
                var query = $"UPDATE employee_info SET `{request.column}` = @Text WHERE `RowNo` = @Row";
                using var command = new MySqlCommand(query, _connection);
                command.Parameters.AddWithValue("@Text", request.text);
                command.Parameters.AddWithValue("@Row", request.row);
        ...
        [HttpPost]
        [Route("deletedata")]
        public async Task<IActionResult> DeleteData([FromBody] DeleteData request)
        {
            var tableName = "employee_info";
            try
            {
                ...
                for (char i = request.startCol; i <= request.endCol; i++)
                {
                ...
                using (var command = new MySqlCommand(query, _connection))
                {
                    command.Parameters.AddWithValue("@StartRow", request.startRow);
                    command.Parameters.AddWithValue("@EndRow", request.endRow);
        ...
        [HttpPost]
        [Route("deleterow")]
        public async Task<IActionResult> DeleteRow([FromBody] AddDeleteRow request)
        {
            var tableName = "employee_info";
            try
            {
                ...
                int rowsDeleted = request.endRow - request.startRow + 1;
                ...
                using (var updateCommand = new MySqlCommand(updateQuery, _connection))
                {
                    ...
                    updateCommand.Parameters.AddWithValue("@EndRow", request.endRow);
        ...
        [HttpPost]
        [Route("addrowabove")]
        public async Task<IActionResult> AddRowAbove([FromBody] AddDeleteRow request)
        {
            var tableName = "employee_info";
            try
            {
                await _connection.OpenAsync();
                int numberOfRows = request.endRow - request.startRow + 1;
                ...
                for (int i = 0; i < numberOfRows; i++)
                {
                    using (var insertCommand = new MySqlCommand(insertQuery, _connection))
                    {
                        ...
                        insertCommand.Parameters.AddWithValue("@RowNo", request.startRow + i);
...
public class DeleteData
{
    ...
    public char startCol { get; set; }
    public char endCol { get; set; }
}
