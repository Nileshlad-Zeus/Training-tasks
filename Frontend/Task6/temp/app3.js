class newCanvas {
    constructor(sheetName) {
      ...
      this.startRow = Math.floor(this.main.scrollTop / 20);
      this.numRows = Math.floor(this.main.offsetHeight / 20);
  
      this.numCols = Math.floor(this.main.offsetWidth / 100);
      this.startCol = Math.floor(this.main.scrollLeft / 100);
    ...
    createNew() {
      ...
      createCanva.width = main.offsetWidth;
      createCanva.height = window.innerHeight - 55;
    ...
    createTopHeader() {
      ...
      createCanva.width = window.innerWidth;
    ...
    createLeftHeader() {
      ...
      createCanva.height = window.innerHeight;
    ...
    scroll() {
      ...
      main.addEventListener("scroll", () => {
        ...
        this.startRow = Math.floor(scrollTop / 20);
        this.numRows = Math.floor(main.offsetHeight / 20);
        ...
        this.numCols = Math.floor(main.offsetWidth / 100);
        this.startCol = Math.floor(scrollLeft / 100);
  
        if (scrollLeft + clientWidth + 5 >= scrollWidth) {
          scroller.style.width = `${scroller.offsetWidth + 200}px`;
          this.clearCanvas();
          this.drawGrid.drawgrid(this.numRows, this.numCols);
        }
  ...
  class getValues {
    ...
    getColumnIndex(x) {
      let cellPosition = 0;
      for (
        let i = this.canvasIns.startCol;
        i <= this.canvasIns.startCol + this.canvasIns.numCols;
        i++
      ) {
    ...
    getRowIndex(x) {
      let cellPosition = 0;
      for (
        let i = this.canvasIns.startRow;
        i <= this.canvasIns.startRow + this.canvasIns.numRows;
        i++
      ) {
  ...
  class drawGrid {
    constructor(ctx, ctxHeader, ctxColHeader, cellWidths, cellHeight, canvasIns) {
      ...
      this.ctx.canvas.addEventListener("mousedown", (e) => {
        ...
        this.startingIndex = [
          this.getValueInstance.getColumnIndex(clickX) - this.canvasIns.startCol,
          this.getValueInstance.getRowIndex(clickY) - this.canvasIns.startRow,
        ];
      });
    ...
    drawRows() {
      let cellPosition = 0;
      const main = document.getElementById("main");
      let startRow = Math.floor(main.scrollTop / 20);
      let noRows = Math.floor(main.offsetHeight / 20);
      for (let y = startRow; y <= startRow + noRows; y++) {
        cellPosition += this.cellHeight.get(y) || 20;
        this.ctx.save();
        this.ctx.beginPath();
    ...
    drawColumns() {
      let cellPosition = 0;
      const main = document.getElementById("main");
      let noCols = Math.floor(main.offsetWidth / 100);
      let startCol = Math.floor(main.scrollLeft / 100);
      for (let x = startCol; x <= startCol + noCols; x++) {
    ...
    renderTopHeader() {
      ...
      for (let x = startCol; x <= startCol + noCols; x++) {
        ...
        let text = this.convertToTitle(x + 1);
        let xPosition = cellPosition - (this.cellWidths.get(x) || 100) / 2;
        let yPosition = 15;
